#!/usr/bin/env python3
"""Simulator page for running a pitch-by-pitch pitcher vs batter at-bat."""
...
def _detailed_outcome_message(entry: dict) -> str:
    ...
    # Build simple direction and distance details for balls in play
...
def _pitch_type_full_name(abbrev: str) -> str:
    """Convert a pitch abbreviation into a readable display name."""
...
# Pitch colors for the strike zone chart
PITCH_TYPE_COLORS = {
    "FF": "#e41a1c",
    "SI": "#377eb8",
    "FC": "#4daf4a",
...
def _pitch_type_color(pt: str) -> str:
    """Return the chart color associated with a pitch type."""
...
def _load_batter_list(_engine):
    ...
    options = []
    batter_info = {}
    # PostgreSQL returns lowercase names for unquoted columns
...
def _initial_game_state():
    """Return the default game state for a new at-bat."""
    return {"balls": 0, "strikes": 0, "outs": 0, "bases": [0, 0, 0], "runs": 0}


def _reset_session_for_ab():
    """Reset Streamlit session state values for a fresh at-bat."""
...
def main():
    """Render the simulator UI and run each pitch step."""
    ...
    # Reset the at-bat when selection changes
    ...
    if st.button("Pitch", type="primary", disabled=pa_ended):
        ...
        # Run one full pitch step and update game state
    ...
    if pitch_log:
        ...
        # Center the graph so it stays compact
